...
var MouseVector = /** @class */ (function () {
    function MouseVector() {
        ...
        this.vector = document.querySelector("#tv");
        window.MouseVector = this;
        window.addEventListener("mousemove", function (e) {
            var mv = window.MouseVector;
            if (!mv.lock) {
                mv.lock = true;
                this.setTimeout(function () {
                    window.MouseVector.lock = false;
                }, 5);
                var X = e.clientX;
                var Y = e.clientY;
                mv.prevX = mv.newX;
    ...
    MouseVector.prototype.delta = function () {
        // this.angle = (Math.sin(this.dY / this.dX)) * (180.0 / Math.PI);
        var trans = "";
        var lim = (this.dX == 1 && this.dY == 0) ||
            (this.dX == -1 && this.dY == 0) ||
            (this.dX == 0 && this.dY == 1) ||
            (this.dX == 0 && this.dY == -1);
        if (!lim) {
            ...
            var length_1 = Math.pow((Math.pow(this.dX, 2) + Math.pow(this.dY, 2)), 0.5);
            this.vector.style.height = (20 + length_1 * 10) + "px";
